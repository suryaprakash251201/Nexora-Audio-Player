...
class PlaylistsApi {
  ...
  Future<List<Playlist>> getPlaylists() async {
    ...
    if (data is Map<String, dynamic>) {
      ...
      else if (data['playlists'] is List) list = data['playlists'] as List;
    ...
    return list.whereType<Map<String, dynamic>>().map((e) => PlaylistDto.fromJson(e).toEntity()).toList();
  }
  ...
  Future<List<Song>> getPlaylistTracks(String id) async {
    ...
    if (data is Map<String, dynamic>) {
      if (data['data'] is List) list = data['data'] as List;
      else if (data['tracks'] is List) list = data['tracks'] as List;
      else if (data['songs'] is List) list = data['songs'] as List;
      else if (data['data'] is Map && data['data']['tracks'] is List) list = data['data']['tracks'] as List;
      else list = [];
    } else if (data is List) {
      list = data;
    } else {
  ...
  Future<Playlist> createPlaylist({required String name, String? description}) async {
    final res = await _client.post(ApiConstants.playlists, data: {
      'name': name,
  ...
  Future<Playlist> updatePlaylist(String id, {String? name, String? description}) async {
    final res = await _client.put(ApiConstants.playlistById(id), data: {
  ...
  Future<void> addTrack(String playlistId, String songId) async {
    ...
    try {
      await _client.post(ApiConstants.playlistTracks(playlistId), data: {'songId': songId});
    } catch (e) {
      // Fallback for servers expecting songIds array
      await _client.post(ApiConstants.playlistTracks(playlistId), data: {'songIds': [songId]});
    }
  }

  Future<void> addTracks(String playlistId, List<String> songIds) async {
    await _client.post(ApiConstants.playlistTracks(playlistId), data: {'songIds': songIds});
  }
  ...
  Future<void> reorder(String playlistId, List<String> orderedIds) async {
    await _client.put(ApiConstants.playlistReorder(playlistId), data: {'orderedIds': orderedIds});
  }
}
