...
class SongsLocalDataSource {
  ...
  Future<List<Song>> getCachedSongs({int limit = 100, int offset = 0}) async {
    final db = await _dbService.database;
    final rows = await db.query('tracks', limit: limit, offset: offset, orderBy: 'updatedAt DESC');
  ...
  Future<List<Song>> searchLocal(String query) async {
    final db = await _dbService.database;
    final rows = await db.query('tracks', where: 'title LIKE ? OR artist LIKE ? OR album LIKE ?', whereArgs: ['%$query%', '%$query%', '%$query%'], limit: 50);
    return rows.map((r) => Song(
          id: r['id'] as String,
          title: r['title'] as String,
          artist: r['artist'] as String?,
