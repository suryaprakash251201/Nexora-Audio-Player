...
class PlaylistsRepository {
  ...
  Future<Playlist> createPlaylist(String name, {String? description}) async {
    ...
    try {
      ...
      if (e is NoInternetException) {
        await _sync.enqueueOperation('CREATE_PLAYLIST', {
          'name': name,
          'description': description,
          'tempId': tempId,
        });
  ...
  Future<void> addTrack(String playlistId, String songId) async {
    ...
    try {
      ...
      final maxOrder = await db.rawQuery(
        'SELECT MAX(sortOrder) as m FROM playlist_items WHERE playlistId = ?',
        [playlistId],
      );
    ...
    try {
      ...
      if (e is NoInternetException) {
        await _sync.enqueueOperation('ADD_TO_PLAYLIST', {
          'playlistId': playlistId,
          'songId': songId,
        });
        return;
  ...
  Future<void> removeTrack(String playlistId, String songId) async {
    ...
    try {
      ...
      if (e is NoInternetException) {
        await _sync.enqueueOperation('REMOVE_FROM_PLAYLIST', {
          'playlistId': playlistId,
          'songId': songId,
        });
        return;
    ...
    try {
      ...
      for (var i = 0; i < orderedIds.length; i++) {
        batch.update(
          ...
          {'sortOrder': i},
          where: 'playlistId = ? AND trackId = ?',
          whereArgs: [playlistId, orderedIds[i]],
        );
      }
      await batch.commit(noResult: true);
    } catch (e) {
      if (e is NoInternetException) {
        await _sync.enqueueOperation('REORDER_PLAYLIST', {
          'playlistId': playlistId,
