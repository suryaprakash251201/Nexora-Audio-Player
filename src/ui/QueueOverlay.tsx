...
export default function QueueOverlay({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    if (visible) {
      ...
      opacity.value = withTiming(0.42, { duration: 300 });
    } else {
  ...
  const panGesture = Gesture.Pan()
    .onUpdate((e) => {
      translateY.value = Math.max(0, e.translationY * 0.8);
    })
    .onEnd((e) => {
  ...
  return (
    <View 
      style={[s.backdrop, { paddingTop: insets.top + 12 }]} 
      pointerEvents={visible ? "box-none" : "none"}
      accessibilityViewIsModal={visible}
    >
      {/* Defocus + dim the screen behind the sheet so the queue owns focus. */}
      {visible && <BlurView intensity={glass.blur.subtle} tint="dark" style={StyleSheet.absoluteFill} />}
      ...
      <GestureDetector gesture={panGesture}>
        <AnimatedGlassSurface 
          ...
          radius={0} 
          border={false} 
          style={[s.sheet, { paddingBottom: insets.bottom + 80 }, sheetStyle]}
        >
          <View style={s.handle} />
          <View style={s.header}>
            <View>
            ...
            <View style={s.headerActions}>
              <Pressable 
                onPress={() => { Haptics.tapLight(); playback.clearQueue(); }} 
                style={s.headerBtn}
